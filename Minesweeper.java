...
public class Minesweeper {
    ...
    public void displayBoard() {
        ...
        for(int i = 0; i < getRows();i++){
            for(int j = 0; j < getCols(); j++){
                System.out.print(getRevealed(i,j) + " ");
            }
            System.out.println();
        }
    ...
    public void playerMove(int row, int col, String action) {
        ...
        if(!gameOver) {
            ...
            else if (!getRevealed(row, col) && action.equals("flag")) {
                flagCell(row, col);
                displayBoard();
            }
            else if ((!getRevealed(row, col) && action.equals("flag"))) {
    ...
    public boolean checkWin() {
        ...
        for (int i = 0; i < getRows(); i++) {
            for (int j = 0; j < getCols(); j++) {
                if(getRevealed(i,j) && !getGameOver()){
                    tracker++;
                }
    ...
    private void revealCell(int row, int col) {
        ...
        char newcell = (char)cellreveal;//casts the int into the character
